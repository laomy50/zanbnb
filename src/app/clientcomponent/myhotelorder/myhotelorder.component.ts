...
@Component({
  selector: 'app-myhotelorder',
  templateUrl: './myhotelorder.component.html',
  styleUrl: './myhotelorder.component.css'
})
export class MyhotelorderComponent {
  ...
  ngOnInit(): void {
    ...
    this.getMyHotelOrders();
  }

  // all rents
  public getMyHotelOrders() {

    const userData = sessionStorage.getItem('logged user');
    const userId = userData ? JSON.parse(userData).userId : null;
    if(userId){ 
      this.bookingService.getHotelByUserId(userId).subscribe(
        (response: any[]) => {
