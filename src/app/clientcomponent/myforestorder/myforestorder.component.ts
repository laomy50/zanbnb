...
@Component({
  selector: 'app-myforestorder',
  templateUrl: './myforestorder.component.html',
  styleUrl: './myforestorder.component.css'
})
export class MyforestorderComponent {
  ...
  ngOnInit(): void {
    ...
    this.getMyForestOrders();
  }

  // all rents
  public getMyForestOrders() {

    const userData = sessionStorage.getItem('logged user');
    const userId = userData ? JSON.parse(userData).userId : null;
    if (userId) {
      this.bookingService.getForestByUserId(userId).subscribe(
        (response: any[]) => {
