...
@Component({
  selector: 'app-myshoporder',
  templateUrl: './myshoporder.component.html',
  styleUrl: './myshoporder.component.css'
})
export class MyshoporderComponent {
  ...
  ngOnInit(): void {
    ...
    this.getMyShopOrders();
  }

  // all rents
  public getMyShopOrders() {

    const userData = sessionStorage.getItem('logged user');
    const userId = userData ? JSON.parse(userData).userId : null;
    if(userId){ 
      this.bookingService.getShoppingByUserId(userId).subscribe(
        (response: any[]) => {
