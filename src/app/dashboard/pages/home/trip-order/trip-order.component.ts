...
if (loggedUser) {
 ...
 this.propertyForm = this.formBuilder.group({
  ...
  tripPackageId: new FormControl('', [Validators.required]),  
    ...
    .subscribe(
      (data) => {
        this.trips = data.map(trip => this.imageProcessingService.createTripImages(trip));
          ...
          // to set the first tripPackageId
          if (this.trips.length > 0) {
            this.propertyForm.get('tripPackageId')?.setValue(this.trips[0].tripPackageId);
          }
      },
      (error) => {
  ...
  openModal(content: TemplateRef<any>, tripPackageId: number): void {
    // Patch the form value with the tripPackageId
    this.propertyForm.patchValue({ tripPackageId: tripPackageId });
    console.log('Selected Trip Package ID:', tripPackageId);
  ...
  bookNow(): void {
    if (this.propertyForm.valid) {
      this.bookingService.createTripBooking(this.propertyForm.value)
        .subscribe(response => {
